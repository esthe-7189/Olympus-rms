...
<div id="botton_position">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="会員登録" onClick="location.href='<%=urlPage%>kaigi/admin/member/writeForm.jsp'">
</div>	
<div id="boxCalendar"  >
<table width="95%" border='0' cellpadding='0' cellspacing='0'>
<form name="search"  action="<%=urlPage%>kaigi/admin/member/listForm.jsp" method="post">
	...
	<td width="50%" align="left">	
		...
		<input type="button"   style=cursor:pointer align=absmiddle value="  目録  " class="cc" onfocus="this.blur();"  onClick="location.href='<%=urlPage%>kaigi/admin/member/listForm.jsp?page=1'">		
	</td>
	...
	<div class="innerDivSlice" >
			<div class="hiddenBox" >	
				...
				<%if(!ingam.equals("no")){%>
						...
						<a href="javascript:openScrollWin('<%=urlPage%>kaigi/admin/member/sign_popView.jsp','read','sign','720','190','&member_id=<%=member.getMember_id()%>&mseq=<%=mseq%>&fileKind=modi');" onfocus="this.blur()">						
						<img src="<%=urlPage%>rms/image/admin/btn_coment_editor.gif"  align="absmiddle"></a>	
				<%}else{%>
						用意されたイメージがありません。<br>
						<a href="javascript:openScrollWin('<%=urlPage%>kaigi/admin/member/sign_popView.jsp','read','sign','720','190','&member_id=<%=member.getMember_id()%>&mseq=<%=mseq%>&fileKind=new');" onfocus="this.blur()">	
	...
	<!--list_inner box----->
		   ...
		   <tr  align="left" onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="" height="25">
			<div class="hidden_div"><input type="hidden" name="mseq<%=mseq%>" id="mseq<%=mseq%>"  value="<%=member.getMseq()%>"></div>
					...
					<td width="5%"  class="clear_dot_with_clide" align="center">																		
						<!--<a href="<%=urlPage%>kaigi/admin/member/delete.jsp?member_id=<%=member.getMember_id()%>&mseq=<%=member.getMseq()%>"  onfocus="this.blur()">-->
...
function goPage(pageNo) {
    document.move.action = "<%=urlPage%>kaigi/admin/member/listForm.jsp";
    document.move.page.value = pageNo;
    document.move.submit();
}
function goModiView(member_id) {	
    document.move.action = "<%=urlPage%>kaigi/admin/member/updateForm_admin.jsp";    
...
function goOkView2(mseq) {		
	...
	document.move.action = "<%=urlPage%>kaigi/admin/member/member_yn.jsp";
...
function goLevelView2(mseq) {	
	...
	document.move.action = "<%=urlPage%>kaigi/admin/member/member_level.jsp";
	document.move.mseq2.value = mseq;
	document.move.okYn.value = valueMseq;
	document.move.submit();
}
function goPosiLevelView2(mseq) {	
	...
	document.move.action = "<%=urlPage%>kaigi/admin/member/member_posisionLevel.jsp";
	document.move.mseq2.value = mseq;
