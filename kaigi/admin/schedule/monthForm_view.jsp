...
<%!
   public String getDateName (int monthNumber) // This method is used to quickly return the proper name of a month
   {
    ...
    DataMgr mgr = DataMgr.getInstance();	    
 	int ydataEnd=0; int mdataEnd=0;int ddataEnd=0; int ydata=0; int mdata=0;int ddata=0;int basho=0;
 	String name=""; String title=""; int fellowCnt=0; String bashoStr=""; String jikan=""; String jikanEnd="";	
%>
...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">会議室予約システム
 </span> 
<div class="clear_line_gray"></div>
<p>
<div id="botton_position">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="リスト" onClick="location.href='<%=urlPage%>kaigi/admin/schedule/listForm.jsp'">
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="カレンダー" onClick="location.href='<%=urlPage%>kaigi/admin/schedule/monthForm.jsp'">		
<!--<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="祝日・休日設定" onClick="location.href='<%=urlPage%>kaigi/admin/schedule/horiForm.jsp'">-->		
</div>
...
<a href="<%=urlPage%>kaigi/admin/schedule/monthForm.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=0"><img src="<%=urlPage%>rms/image/admin/btn_calendar_ridou_01.gif" ></a>
&nbsp;
<%=cal.get(cal.YEAR)%>年<%=getDateName (cal.get(cal.MONTH))%>月
&nbsp;
<a href="<%=urlPage%>kaigi/admin/schedule/monthForm.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=1"><img src="<%=urlPage%>rms/image/admin/btn_calendar_ridou_02.gif" ></a>
</td>
	...
	int i=1; 
		Iterator listiterCol=listDuring.iterator();					
				while (listiterCol.hasNext()){
					...
					jikan=duringdata.getJikan_begin()+":"+duringdata.getBun_begin();
					jikanEnd=duringdata.getJikan_end()+":"+duringdata.getBun_end();	
					List listFellowdu=mgr.listFellow_seq(seq);
%>
<c:set var="name" value="<%=name%>"/> 
<c:set var="title" value="<%=title%>"/>
<c:set var="jikan" value="<%=jikan%>"/>
<c:set var="jikanEnd" value="<%=jikanEnd%>"/>
<c:set var="listFellowdu" value="<%=listFellowdu%>"/>
<c:set var="bashoStr" value="<%=bashoStr%>"/>
...
<%if(duringdata.getMember_id().equals("moriyama")){%>	
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>							
				<div class="colorBar_begin" style="background:#2892BC; color:#ffffff; margin-top:0px;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; margin-top:0px;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">																
		...
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#2892BC; color:#ffffff; margin-top:0px;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">	
		<%}else{%>	
			<div class="colorBar_middle" style="background:#2892BC; color:#ffffff; margin-top:0px;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">				
		<%}%>
<%}else if(duringdata.getMember_id().equals("juc0318")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#CE9000; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">	
			<%}%>						
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#CE9000; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#CE9000; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>	
<%}else if(duringdata.getMember_id().equals("biofloc")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#FF41A7; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">	
			<%}%>										
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#FF41A7; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#FF41A7; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>	
<%}else if(duringdata.getMember_id().equals("tachi")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#F0857D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style=" color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">		
			<%}%>										
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#F0857D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#F0857D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>				
<%}else if(duringdata.getMember_id().equals("lin")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#128D3D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">	
			<%}%>											
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#128D3D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#128D3D; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>
<%}else if(duringdata.getMember_id().equals("hazama")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#8328A1; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">	
			<%}%>										
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#8328A1; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#8328A1; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>
<%}else if(duringdata.getMember_id().equals("funakubo")){%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#01A79B; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">		
			<%}%>										
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#01A79B; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#01A79B; color:#ffffff; "  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">		
		<%}%>									
<%}else{%>
		<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>					
				<div class="colorBar_begin" style="background:#DD1E3A; color:#ffffff;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/admin/icon_cal_begin.gif" align="absmiddle">
			<%}else{%>
				<div class="colorBar_begin" style="color:#666666;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>) " onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">
				<img src="<%=urlPage%>rms/image/icon_s.gif" align="absmiddle">		
			<%}%>										
		<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
			<div class="colorBar_end" style="background:#DD1E3A; color:#ffffff;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
			<img src="<%=urlPage%>rms/image/admin/icon_cal_end.gif" align="absmiddle">
		<%}else{%>
			<div class="colorBar_middle" style="background:#DD1E3A; color:#ffffff;"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">	
		<%}%>	
<%}%>		
	<%if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata){%>			
			<%if(ddata!=ddataEnd){%>							
				<span class="schenav">
					<c:if test="${fn:length(bashoStr)>=10}"> ${fn:substring(bashoStr, 0, 10)}</c:if>
					<c:if test="${fn:length(bashoStr)<10}"> ${bashoStr}</c:if> (${jikan}~${jikanEnd} /${fn:substring(name, 0, 1)})
				</span>
			<%}else{%>
				<span class="topnav">
					<c:if test="${fn:length(bashoStr)>=10}"> ${fn:substring(bashoStr, 0, 10)}</c:if>
					<c:if test="${fn:length(bashoStr)<10}"> ${bashoStr}</c:if> <font color="#007AC3">(${jikan}~${jikanEnd} /${fn:substring(name, 0, 1)})</font>
				</span>				
			<%}%>
	<%}else if(cal.get(cal.YEAR)==ydataEnd && cal.get(cal.MONTH)==mdataEnd-1 && dispDay==ddataEnd){%>
				<span class="schenav">
					<%if(!duringdata.getDuring_begin().equals(duringdata.getDuring_end())){%>
						<c:if test="${fn:length(bashoStr)>=10}"> ${fn:substring(bashoStr, 0, 10)}</c:if><c:if test="${fn:length(bashoStr)<10}"> ${bashoStr}</c:if>(${jikan}~${jikanEnd}/${fn:substring(name, 0, 1)}) 
					<%}%>	
				</span>	
	<%}%>		
	...
	int i=1; int ifel=0; 	
		Iterator listiter=list.iterator();					
				while (listiter.hasNext()){
					...
					jikan=dbbean.getJikan_begin()+":"+dbbean.getBun_begin();
					jikanEnd=dbbean.getJikan_end()+":"+dbbean.getBun_end();																
					...
					List listFellow=mgr.listFellow_seq(seq);
...
if(cal.get(cal.YEAR)==ydata && cal.get(cal.MONTH)==mdata-1 && dispDay==ddata ){	%>		    	
		...
		<a class="topnav"  href="javascript:FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');" style="cursor:pointer;" onfocus="this.blur()">
			<div   title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','cal','<%=currMonth+1%>','<%=currYear%>');">						
			<%if(viewCnt==1){%>
				<img src="<%=urlPage%>rms/image/admin/icon_taikin_schdule.gif" align="absmiddle">
				<%if(bashoStr.length()>10){%><%=bashoStr.substring(0,10)%>
				<%}else{%><%=bashoStr%><%}%>				
				<font color="#007AC3">(<%=jikan%>~<%=jikanEnd%> /<%=name.substring(0,1)%>)</font>				
			<%}%>			
			<%if(viewCnt==2){%>
			...
			<%if(viewCnt==3 || viewCnt==4 || viewCnt==5){%>
				...
				<%if(bashoStr.length()>10){%><%=bashoStr.substring(0,10)%>
				<%}else{%><%=bashoStr%><%}%>
				<font color="#007AC3">(<%=jikan%>~<%=jikanEnd%></font><font color="#999900"><%=name.substring(0,1)%>)</font>				
			<%}%>					
		</div>
		</a>									
...
function prdListView_pop_open(yVal,mVal,dVal,pg,monthVal,yearVal){
	...
	if(dVal.length==1){
	 ...
	 if(document.getElementById("layerpop").style.display == 'none'){
		...
		iframe_inner.location.href = "<%=urlPage%>kaigi/admin/schedule/ZoomDetailPopup.jsp?yVal="+yVal+"&mVal="+mVal+"&dVal="+dValue+"&pg="+pg+"&monthVal="+monthVal+"&yearVal="+yearVal;
	 } else{
...
function FuncDetailPopupSeq(ymdList,seq,pg,monthVal,yearVal){
	 if(document.getElementById("layerpop").style.display == 'none'){
		document.getElementById("layerpop").style.display="block";		
		iframe_inner.location.href = "<%=urlPage%>kaigi/admin/schedule/ContentDetailSeqPopup.jsp?ymdList="+ymdList+"&seq="+seq+"&pg="+pg+"&monthVal="+monthVal+"&yearVal="+yearVal; 
	 } else{
	 	 iframe_inner.location.replace("about:blank");
	 	document.getElementById("layerpop").style.display = "none";
	 }	 	
...
function doSubmitYM(){
	...
	location.href ="<%=urlPage%>kaigi/admin/schedule/monthForm.jsp?month="+im_sMonth+"&year="+im_sYear+"&action=0";
}
</script>
