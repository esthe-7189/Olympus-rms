...
<%!
   public String getDateName (int monthNumber) // This method is used to quickly return the proper name of a month
   {
    ...
    DataMgr mgr = DataMgr.getInstance();	    
 	...
 	String name=""; String title=""; String endYMD; int fellowCnt=0;String bashoStr=""; String jikan=""; String jikanEnd="";
...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">会議室予約システム</span> 
...
<a href="<%=urlPage%>kaigi/admin/schedule/listForm.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=0"><img src="<%=urlPage%>rms/image/admin/btn_calendar_ridou_01.gif" ></a>
&nbsp;
<%=cal.get(cal.YEAR)%>年<%=getDateName (cal.get(cal.MONTH))%>月
&nbsp;
<a href="<%=urlPage%>kaigi/admin/schedule/listForm.jsp?month=<%=currMonth%>&year=<%=currYear%>&action=1"><img src="<%=urlPage%>rms/image/admin/btn_calendar_ridou_02.gif" ></a>
</td>
...
%> 		
		...
		<div class="topnav"  title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq%>','list','<%=currMonth+1%>','<%=currYear%>');">		
			...
			<%=bashoStr%><font color="#007AC3">(<%=jikan%>~<%=jikanEnd%>/<%=name%>)</font>&nbsp;&nbsp;&nbsp;
		</div></a>
	...
	int ii=1;
		Iterator listiter2=list2.iterator();					
				while (listiter2.hasNext()){
					...
					jikan=dbbean2.getJikan_begin()+":"+dbbean2.getBun_begin();
					jikanEnd=dbbean2.getJikan_end()+":"+dbbean2.getBun_end();							
...
<div class="topnav"   title="<%=bashoStr%>(<%=jikan%>~<%=jikanEnd%>/<%=name%>)" onclick="FuncDetailPopupSeq('<%=ymdList%>','<%=seq2%>','list','<%=currMonth+1%>','<%=currYear%>');">					
	<%if(viewCnt==0){%><img src="<%=urlPage%>rms/image/icon_s.gif">
	<%}else if(viewCnt==1){%><img src="<%=urlPage%>rms/image/admin/icon_taikin_schdule.gif" align="absmiddle">
	<%}else if(viewCnt==2){%><img src="<%=urlPage%>rms/image/admin/icon_jangyo_01.gif" align="absmiddle">
	<%}else if(viewCnt==3 || viewCnt==4 || viewCnt==5){%><img src="<%=urlPage%>rms/image/admin/icon_hokoku.gif" align="absmiddle"><%}%>
...
<%=bashoStr%>
&nbsp;
<font color="#007AC3"><%=jikan%>~<%=jikanEnd%></font><font color="#999900"></font>&nbsp;<font color="#CC66FF">(終了)</font>
...
function doSubmitYM(){
	...
	location.href ="<%=urlPage%>kaigi/admin/schedule/listForm.jsp?month="+im_sMonth+"&year="+im_sYear+"&action=0";
}
</script>