...
<%	
String name=""; String mailadd=""; String pass=""; int mseq=0; int level=0; String position=""; int dbPosiLevel=0; String busho="";
...
int month= Integer.parseInt(mgrYM.substring(5,7));
...
//출퇴근 시간 (일반인 8시45~17시30분 총 8시간 근무, 점심 45분/  계약직 현재 id hazama는 8시15~06시30분 총 )	
...
<%
	...
	int dispDay = 1;	
	String ymdToday="";
	int totalsumH=0; int totalsum=0; int totalKyusum=0;int totalChikokuTime=0;	
	for (int w = 1; w < 7; w++){						
  		for (int d = 1; d < 8; d++){  		  	  			
	...
	String beginZero="0"; String endZero="0"; String riyu=""; String comment="&nbsp;";
	int totalValueHH_s=0; int totalValueMM_s=0;	  String daikyu="0"; String daikyu_date="0"; int chikoku_hh=0; int chikoku_mm=0; int begin_hh=0; int end_hh=0;int horiTotal=0;
...
if(dbb !=null){	
		...
		oneday_holi=dbb.getOneday_holi();
					...
					// 잔교시 휴식 15분	
			if(jobTimeKubun.equals("holi") && seq!=0){
			...
			if(jobTimeKubun.equals("day") && seq!=0){
...
<%if(seq!=0){%>
	...
	<td  style="padding:0 0 0 0" align="center" bgcolor="<%=bgColElse%>" class="tFont" rowspan="2">		
		...
		<%}else if(jobTimeKubun.equals("day") && seq !=0){%>
					<%if(oneday_holi.equals("0") && daikyu.equals("0")){%>
							<%=totalValueHH_s%>:<%if(totalValueMM_s==0){%><%=totalValueMM_s%>0<%}else{%><%=totalValueMM_s%><%}%> 					
					<%}else if(oneday_holi.equals("0") && !daikyu.equals("0")){%>
							&nbsp;					
					<%}else if(!oneday_holi.equals("0") && daikyu.equals("0")){%>
							&nbsp;					
					<%}else{%>
...
<%
	...
	%>
		...
		<%if(sign_ok==3 ){%><font color="#007AC3"><%=memSign.getNm()%></font><br><font color="#BA7474">差戻し</font><%}%> 
	<%}}else{%>
		&nbsp;			
	<%}%>	
	</td>			
...
<%	count += 1;
	dispDay += 1;	
	totalsumH +=totalValueHH_s;	
	totalsum +=totalValueMM_s;
				}				
			}
...
int daytotal=(totalsumH*60)+totalsum;
%>	
...
<tr  align=center  onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">
	...
	<td align="center" class="calendar5" ><%=daytotal/60%>:<%if(daytotal%60==0){%><%=daytotal%60%>0<%}else{%><%=daytotal%60%><%}%></td>
