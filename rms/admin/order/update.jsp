...
manager.updatePurchase_order(order);	

//delete items 
manager.deleteItem(Integer.parseInt(seq));
...
if(product_nm.length==1){			
	order.setParent_seq(Integer.parseInt(seq));
	order.setProduct_nm(product_nm[0]);
	order.setOrder_no(order_no[0]);
	order.setProduct_qty(Integer.parseInt(product_qty[0]));
	order.setUnit_price(Integer.parseInt(unit_price[0]));
	order.setClient_nm(Integer.parseInt(client_nm[0]));
