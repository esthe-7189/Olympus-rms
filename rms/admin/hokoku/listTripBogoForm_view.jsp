	...
	if (searchCond != null && searchCond.length > 0 && searchKey != null){
		for (int i=0;i<searchCond.length ;i++ ){
			if (searchCond[i].equals("pcode")){
				whereCond.add(" b.nm LIKE '%"+searchKey+"%'");				
...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">出張/休日勤務 > 出張報告書 > 全体目録 </span> 
...
<table width="95%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">	
	<tr>			
		<td style="padding: 0px 0px 0px 10px" ><img src="<%=urlPage%>orms/images/common/jirusi.gif" align="absmiddle"> 
			<span class="calendar7">出張報告書</span>			
    		</td> 		
		<td align="right">
...
<div id="boxNoLineBig"  >
	<div id="searchBox">				
		...
		<table  border="0" cellpadding="5" cellspacing="5" width="95%" >
			<form name="searchDate"   action="<%=urlPage%>rms/admin/hokoku/listTripBogoForm.jsp" method="post"  >		
...
<table width="100%">
	<form name="search"   action="<%=urlPage%>rms/admin/hokoku/list/listTripBogoForm.jsp" method="post"  >		
	...
	<tr>
		...
		<%if(id.equals("moriyama") || id.equals("juc0318") || id.equals("admin")){%>			
			<td align="right">
						<a class="topnav"  href="javascript:goJumpTripBogo('1');" onfocus="this.blur();">[品質管理部]</a>
						<a class="topnav"  href="javascript:goJumpTripBogo('2');" onfocus="this.blur();">[製造部]</a>
						<a class="topnav"  href="javascript:goJumpTripBogo('3');" onfocus="this.blur();">[管理部]</a>
						<a class="topnav"  href="javascript:goJumpTripBogo('0');" onfocus="this.blur();">[経営役員]</a>
						<a class="topnav"  href="javascript:goJumpTripBogo('4');" onfocus="this.blur();">[その他]</a>
		    </td>			
		<%}%>				
	</tr>
	</form>	
...
<tr bgcolor=#F1F1F1 class="tablebox_list" cellpadding="1" cellspacing="1" align=center height=26>	
	<td  align="center" width="11%" class="title_list_all">日付</td>		
    <td  align="center" width="10%" class="title_list_m">所属</td>
    <td  align="center" width="13%" class="title_list_m">出張者氏名</td>
    <td  align="center" width="20%" class="title_list_m">出張期間</td>    
    <td  align="center" width="20%" class="title_list_m">出張先</td>
    <td  align="center" width="5%" class="title_list_m">承認(1)</td>
    <td  align="center" width="5%" class="title_list_m">承認(2)</td>
    <td  align="center" width="8%" class="title_list_m">修正/削除</td>
...
<tr onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">
	...
	<td class="clear_dot">
		<a href="javascript:ShowHidden('itemData_block','<%=i%>');" onFocus="this.blur()">
				<font color="#009900"><%=pdb.getDuring_begin()%>～<%=pdb.getDuring_end()%></font></a>
	</td>		
	...
	<%
		...
		<%}}else{%>--<%}%>						
			...
			<tr  height="29">
				...
				<td align="left" style="padding: 0px 0px 0px 2px"  bgcolor="#ffffff" width="45%"><%=pdb.getNm()%></td>
    		...
    		<tr  height="29">
				...
				<td style="padding: 0px 0px 0px 2px" bgcolor="#ffffff" colspan="3" align="left"><%=pdb.getDestination()%></td>
			</tr>		
    		<tr  height="29">
				<td align="center" bgcolor="#f7f7f7">出張期間</td>
				<td style="padding: 0px 0px 0px 2px" bgcolor="#ffffff" colspan="3" align="left"><%=pdb.getDuring_begin()%> ～ <%=pdb.getDuring_end()%></td>				
    		</tr>			
			<tr  height="29">
				<td align="center" bgcolor="#f7f7f7">出張目的</td>
				<td style="padding: 0px 0px 0px 2px" bgcolor="#ffffff" colspan="3" align="left"><%=pdb.getReason()%></td>				
			</tr>	
			<tr >
				<td style="padding: 0px 0px 0px 2px" align="left" bgcolor="#ffffff" colspan="4">実施事項<br><br>
					<%=pdb.getComment()%>					
				</td>											
			</tr>
			<tr>
				<td align="left" bgcolor="#ffffff" colspan="4" style="padding: 2px 2px 2px 5px">
...
function goPage(pageNo) {    
	...
	document.move.action = "<%=urlPage%>rms/admin/hokoku/listTripBogoForm.jsp";
...
function goJumpWrite(busho) {    
	document.searchDate.bushopg.value =busho;    
	document.searchDate.action = "<%=urlPage%>rms/admin/hokoku/writeTripBogo/writeTripForm.jsp";
    document.searchDate.submit();
}
...
function goDelete(fno,ymd) {
	...
	document.move.action = "<%=urlPage%>rms/admin/hokoku/writeTripBogo/delete.jsp";
	document.move.fno.value=fno;	
    document.move.submit();
}
function goModify(fno) {	
    document.move.action = "<%=urlPage%>rms/admin/hokoku/writeTripBogo/updateForm.jsp";
	document.move.fno.value=fno;	
    document.move.submit();
}
function popPrint(fno){				
	var param="&fno="+fno;
	openScrollWin("<%=urlPage%>rms/admin/hokoku/excelForms/formBogo.jsp", "signAdd", "signAdd", "500", "700",param);
}
function goExcel(fno) {	
    document.move.action = "<%=urlPage%>rms/admin/hokoku/excelForms/formBogoExcel.jsp";
	document.move.fno.value=fno;	
    document.move.submit();
}
</script>
