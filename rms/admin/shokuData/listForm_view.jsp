...
*/
      ...
      int count = manager.count(whereCond, whereValue,  Integer.parseInt(pg));
...
//	List  list=manager.selectList(whereCond, whereValue,startRow,endRow, mseq,pageArrow);	
	List  list=manager.selectList(whereCond, whereValue,startRow,endRow, Integer.parseInt(pg));
//대,중분류 호출	
...
<c:set var="cateMenu" value="<%= cateMenu %>" />
...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">月報・週報開発/QMS<font color="#A2A2A2">&nbsp;>&nbsp;</font>${cateMenu.name}</span> 
<div class="clear_line_gray"></div>
<p>
<div id="botton_position">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="開発会議/QMSメイン" onClick="location.href='<%=urlPage%>rms/admin/shokuData/mainForm.jsp'">			
...
<form name="move" method="post">
	...
	<tr>
	<form name="search"  action="<%=urlPage%>rms/admin/shokuData/listForm.jsp?pg=<%=pg%>" method="post">
		...
		<td valign="middle" width="13%">			
		 ...
		 <td width="59%"  valign="bottom"  align="right" style="padding:5 0 5 15">		
		 	&nbsp;				
	    </td>				
	</tr>
	</form>
...
<table width="975"  cellpadding="2" cellspacing="2" class="tablebox_list">
			<tr bgcolor=#F1F1F1 align=center height=26>			
				<td  width="12%" class="clear_dot">大分類</td>
...
%>		
	...
	<%}%>				
				...
				<td class="clear_dot"><%=job.getName()%></td>				
				<td  align="left" class="clear_dot">	<%=job.getTitle()%>&nbsp;</td>				
				<td align="left" class="clear_dot"><a class="fileline" href="javascript:goDown2('<%=job.getFile()%>')"  onfocus="this.blur()">
					<%=job.getFile()%>
					</a>
				</td>
...
function goPage(pageNo) {
    document.move.action = "<%=urlPage%>rms/admin/shokuData/listForm.jsp";
