...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">製造記録書QA </span> 
...
<div id="botton_position">	
	...
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="データ出力(Excel)" onClick="location.href='<%=urlPage%>rms/admin/seizo/excelList_view.jsp'">		
</div>
<div id="boxNoLineBig"  >
	<div id="searchBox">				
		<div class="searchBox_all">	
		<table  border="0" cellpadding="5" cellspacing="5" width="95%" >
			<form name="searchDate"   action="<%=urlPage%>rms/admin/seizo/listForm.jsp" method="post"  >
				<tr>
...
<div class="clear_margin"></div>
 <table width="960">
 	 <form name="search"  action="<%=urlPage%>rms/admin/seizo/listForm.jsp" method="post">	
		<tr>
			...
			<td width="50%" align="right">	
				...
				<input type="submit" class="cc" onfocus="this.blur();" style=cursor:pointer value="検索">
				<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="全体リスト" onClick="location.href='<%=urlPage%>rms/admin/seizo/listForm.jsp'">
			</td>
		</tr>
	</form>
</table>	
...
<tr height=26>		
	<td  align="center" width="4%"><font  color="#FF6600">(<%=count%>個)</font></td>
    	<td  align="center" width="96%"　colspan="1５"><span class="calendar7"> 製造記録書</span> </td>
</tr>
</table>
    ...
    <td  width="15%">
		<table width=100% class="tablebox_list" cellpadding="1" cellspacing="1" >
			<tr  align=center height=26>	
				<td  align="center"  class="clear_dot_gray">記録書原文(ORMS)</td>
			</tr>			
...
<%}%>
	...
	<%}%>
		...
		<%if (pdb.getKind_yn()==0){%>
			<a href="javascript:openScrollWin('<%=urlPage%>rms/admin/seizo/kindCh_pop.jsp','read','상세보기','720','290',
		...
		<a href="javascript:openScrollWin('<%=urlPage%>rms/admin/seizo/commentList_pop.jsp','popUpdate','상세보기','720','400',
...
<c:if test="${! empty  kindbun}" >		
	...
	<%}%>		
		...
		<a href="javascript:openScrollWin('<%=urlPage%>rms/admin/seizo/modifyFormLevel2_pop.jsp','popUpdate','상세보기','720','290',
...
function goPage(pageNo) {    
	...
	document.move.action = "<%=urlPage%>rms/admin/seizo/listForm.jsp";
    	document.move.submit();
}
function goDelete(fno) {
	document.move.action = "<%=urlPage%>rms/admin/seizo/bunshoDelForm.jsp";
	document.move.fno.value=fno;	
    	document.move.submit();
}
function goModify(fno) {	
    	document.move.action = "<%=urlPage%>rms/admin/seizo/modifyForm.jsp";
	document.move.fno.value=fno;	
    	document.move.submit();
}
function goModifyLevel_2(fno,file_kind,bseq) {					
			var param="&fno="+fno+"&file_kind="+file_kind+"&bseq="+bseq;
			openNoScrollWin("<%=urlPage%>ms/admin/seizo/modifyFormLevel2_pop.jsp", "update", "update", "720", "290", param);					
}
function goDeleteLevel2(fno,file_kind,bseq,parentId) {		
    	document.move.action = "<%=urlPage%>rms/admin/seizo/bunshoDelLevel2Form.jsp";
	document.move.fno.value=fno;	
...
function goDown2(filename) {		
    	document.move.action = "<%=urlPage%>rms/admin/seizo/down.jsp";
...
function goComment(fno,file_kind,bseq) {					
			...
			openNoScrollWin("<%=urlPage%>ms/admin/seizo/comentList_pop.jsp", "update", "update", "720", "290", param);					
}
