...
Calendar calEndBeforeYear = Calendar.getInstance();       	   								
    calEndBeforeYear.add(calEndBeforeYear.YEAR, -10);
...
<!--제조/품질 게시판 begin*************************** -->				
		...
		<img src="<%=urlPage%>rms/image/admin/main/qtboardTitle.jpg" align="absmiddle" ></a>
...
<!--사내 게시판 begin*************************** -->	
		...
		<img src="<%=urlPage%>rms/image/admin/main/5s_board_title.jpg" align="absmiddle" ></a>
	<div class="boxTable_eee">		
		<table width="100%"  cellpadding="2" cellspacing="2">
			<tbody>																		
...
function cdtd() {	
	...
	<c:if test="${empty listGmp2 && empty listGmpNo && listGmp4}">		
...
<%            
			...
			Iterator listiter=listGmpNo.iterator();
				while (listiter.hasNext()){				
					...
					String giginm=db_item2.getGigi_nm();
		...
		%>
			...
			<tr onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">			
				...
				<%if(date02 !=null){%><%=date02.substring(0,10)%><%}else{%>0000-00-00<%}%></font><img src="<%=urlPage%>rms/image/admin/main/attentionGmpNo.gif" align="asbmiddle" width="33" height="13"></td>
				<td align="center" ><font color="#CC0000"><%=kanri_no%></font></td>																		
				<td ><a class="fileline" href="javascript:goRead(<%=bseq%>)"  onfocus="this.blur()">
				<%if(giginm !=null){%><%=giginm%><%}else{%>--<%}%></a> </td>		
			       <td ><%if(seizomoto !=null){%><%=seizomoto%><%}else{%>--<%}%></td>							
				<td align="center">
      			<font color="#FF6600">[使用不可]</font>
	         		<%if(GmpModi==1){%>
...
<!---GMP 사용불가만 출력 end-->
<!---GMP2 70일전까지 출력 begin-->													
	<c:if test="${! empty listGmp2}">
	<tr height="10"><td colspan="5" ><font color="#00a0f3">------------------------70日前 [<%=today%>]～[<%=strDateEnd%>]------------------------</font></td></tr>	
<%        
...
<!---GMP2 70일전까지 출력 end-->
<!---GMP4 지난 데이터 출력 begin-->													
	<c:if test="${! empty listGmp4}">
	<tr height="10"><td colspan="5" ><font color="#00a0f3">-----------------------------[<%=today%>]の以前---------------------------------</font></td></tr>	
<%        
			int i=1;	String date02Val="";						
			Iterator listiter=listGmp4.iterator();
				while (listiter.hasNext()){				
					GmpBeen db_item4=(GmpBeen)listiter.next();
					int bseq=db_item4.getBseq();	
					
					if(db_item4.getDate02()!=null && db_item4.getDate02_yn()==1){
						if(db_item4.getDate02().length()>10){
							date02Val=db_item4.getDate02().substring(0,10);
						}else if(db_item4.getDate02().length()==10){
							date02Val=db_item4.getDate02();
						}				
					}else{						
	%>
				<tr height="20"><td colspan="5" >---</td></tr>	
	...
	if(db_item4.getDate02_yn()==1){							
	%>						
		<input type="hidden" name="gmpdate" id="gmpdate" value="<%=date02Val%>-<%=bseq%>-2">						
			<tr onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">			
				<td ><font color="#CC0000"><%=db_item4.getDate02().substring(0,10)%></font><img src="<%=urlPage%>rms/image/admin/main/attentionGmp.gif" align="asbmiddle" width="33" height="13"></td>
				<td align="center" ><font color="#CC0000"><%=db_item4.getKanri_no()%></font></td>																		
				<td ><a class="fileline" href="javascript:goRead(<%=bseq%>)"  onfocus="this.blur()"><%if(db_item4.getGigi_nm()!=null){%><%=db_item4.getGigi_nm()%><%}else{%>&nbsp;<%}%>&nbsp;</a> </td>											
		
			       <td ><%if(db_item4.getSeizomoto()!=null){%><%=db_item4.getSeizomoto()%><%}else{%>&nbsp;<%}%>			       	   
			      </td>							
				<td align="center">
      			<%if(db_item4.getEda_no()==1 ){%> [未実施] <%}else{ %>[使用不可] <%}%> 
