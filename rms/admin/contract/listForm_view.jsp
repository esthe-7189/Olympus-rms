...
<div id="searchBox">
	<div class="searchBox_left">
		<form name="searchCate"  action="<%=urlPage%>rms/admin/contract/listForm.jsp" method="post">		
	...
	<form name="searchKind"  action="<%=urlPage%>rms/admin/contract/listForm.jsp" method="post">			
...
<form name="search"  action="<%=urlPage%>rms/admin/contract/listForm.jsp" method="post">
...
<div class="searchBoxTwo_left50">				
	<form name="searchKousin"  action="<%=urlPage%>rms/admin/contract/listForm.jsp" method="post">
...
<div class="searchBoxTwo_right50">	
	<form name="searchDate"   action="<%=urlPage%>rms/admin/contract/listForm.jsp" method="post"  >				
...
<table width="98%"  cellpadding="0" cellspacing="0" >	 	 	
	...
	　　<td width="5%"  class="title_list_m_r">契約形態</td>		    
		...
		<td  width="10%"  class="title_list_m_r">契約書</td>		
		<td  width="4%" class="title_list_m_r">修正<br>削除</td>		
		<td  width="4%"  class="title_list_m_r">確認</td>				
	</tr>
...
%>								
	...
	<tr height=25 onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">	    
	    ...
	    <td  class="line_gray_bottomnright">
			...
			<a class="fileline" href="#" onclick="popupSeq('<%=seq%>','<%=db.getFile_nm()%>');"   onfocus="this.blur()"><%=db.getFile_nm()%></a>
...
function goPage(pageNo) {    
	...
	document.move.action = "<%=urlPage%>rms/admin/contract/listForm.jsp";
    	document.move.submit();
}
function goDelete(code,seq,filename) {
	if(confirm(code+"のデータを削除しますか?")!=1){return;}
	document.move.action = "<%=urlPage%>rms/admin/contract/delete.jsp";
...
//	var passValue=document.getElementById("passValue_"+seq).value;
	document.move.action = "<%=urlPage%>rms/admin/contract/down.jsp";
	document.move.seq.value = seq;	
	document.move.filename.value = filename;
...
function excelDown(){
	document.move.action = "<%=urlPage%>rms/admin/contract/listExcel.jsp";	
    	document.move.submit();    	
}
function excelExplain(){		
	openNoScrollWin("<%=urlPage%>rms/admin/contract/excelDown_pop.jsp", "契約書", "契約書", "760", "525","");
}
function printDown(){			
	openScrollWin("<%=urlPage%>rms/admin/contract/print_pop.jsp", "契約書", "契約書", "760", "700","");	
}
...
function popupSeq(seq,filename){	
	...
	//overlay.style.opacity = .8;
	 ...
	 if(document.getElementById("passpop").style.display == 'none'){
		...
		iframe_inner.location.href = "<%=urlPage%>rms/admin/contract/popup_downView.jsp?seq="+seq+"&filename="+filename; 
	 } else{
...
function popupOnOff(seq){	
	...
	//overlay.style.opacity = .8;		
	 ...
	 if(document.getElementById("passpop").style.display == 'none'){
	 	 overlay.style.display = "block";
		document.getElementById("passpop").style.display="block";		
		iframe_inner.location.href = "<%=urlPage%>rms/admin/contract/popup_OnOff.jsp?seq="+seq+"&mseq=<%=mseq%>&pgkind=list"; 
	 } else{
	 	 iframe_inner.location.replace("about:blank");
	 	 overlay.style.display = "none";
	 	document.getElementById("passpop").style.display = "none";
...
function doEnter(){
	...
	frm.action = "<%=urlPage%>rms/admin/contract/listForm.jsp";	
	frm.submit();
}
</script>
