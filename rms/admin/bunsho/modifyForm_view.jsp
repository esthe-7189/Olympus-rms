...
BunshoBean bunData=manager.getBunsho(Integer.parseInt(fno));
...
var frmCate=document.memberInput;
	...
	with (document.memberInput) {
	 ...
	 if ( confirm("修正しますか?") != 1 ) { return; }	
     	frmCate.action = "<%=urlPage%>rms/admin/bunsho/modifyUpload.jsp";	
	frmCate.submit();  
}
...
<img src="<%=urlPage%>rms/image/icon_ball.gif" style="filter:Alpha(Opacity=30);"><span class="calendar7">CHONDRON文書 <font color="#A2A2A2">></font> 編集</span> 
...
<div id="botton_position">	
	...
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="データ出力(Excel)" onClick="location.href='<%=urlPage%>rms/admin/bunsho/listForm.jsp'">	
</div>
...
<form name="memberInput" action="<%=urlPage%>rms/admin/bunsho/modifyUpload.jsp" method="post" enctype="multipart/form-data" >	
	...
	<tr >
		<td align="" ><img src="<%=urlPage%>rms/image/icon_s.gif" >翻訳段階:</td>
		<td>

				<input type="radio" name="file_kind"  value="1"  onfocus="this.blur()"   checked>韓国原文(SW) &nbsp;-->&nbsp;
				<input type="radio" name="file_kind"  value="2" onfocus="this.blur()" disabled>翻訳初本(NH)&nbsp;-->&nbsp;
				<input type="radio" name="file_kind"  value="3" onfocus="this.blur()" disabled>PG完成本(PG)&nbsp;-->&nbsp;
				<input type="radio" name="file_kind"  value="4" onfocus="this.blur()" disabled>ORMS最終本(OR)&nbsp;
