...
String kubun_bseq = request.getParameter("kubun_bseq");
...
if(kubun_bseq==null){
	...
	kubun_bseqint=Integer.parseInt(kubun_bseq);
}
...
ContractMgr mana = ContractMgr.getInstance();
 ...
 ContractBeen codeItem=mana.getCodeLimit(kubun_bseqint);
 if (codeItem != null) {                
 ...
 List listCode=mana.listCode(kubun_bseqint); 
 ...
 String catenm="";
 Category catedb=mgrCate.getCateParent(kubun_bseqint); 
 	 if(catedb!= null){ 
 	 	catenm=catedb.getName();		 
	}
%>
...
function codeView() {	
  var frmCate = document.kubun;
  var frm = document.forminput;		
  var valueBseq = frmCate.kubun_noVal.options[frmCate.kubun_noVal.selectedIndex].value;
  var valName = frmCate.kubun_noVal.options[frmCate.kubun_noVal.selectedIndex].text;        	      
	location.href = "addForm.jsp?kubun_bseq="+valueBseq;		
}
function formSubmit(){        
	  var frmCate = document.kubun;
	  var frm = document.forminput;			
	  var parentIdempty=frmCate.kubun_noVal.options[frmCate.kubun_noVal.selectedIndex].value;
	  var nameMseq=frm.sekiningNm.options[frm.sekiningNm.selectedIndex].value;			  	  
		  
	   frm.kubun_bseq.value=frmCate.kubun_noVal.options[frmCate.kubun_noVal.selectedIndex].value; 	
	   frm.parentId.value=parentIdempty;
	  if(isEmpty(frmCate.kubun_noVal, "契約区分を選択して下さい。")) return ; 
...
//	  if(isEmpty(frm.fileNm, "契約書を選択して下さい。")) return ;	 	  	  	  
	...
	frm.sekining_nm.value =frm.sekiningNm.options[frm.sekiningNm.selectedIndex].text;	  
	 ...
	 if(frm.fileNm.value==""){
	  ...
	  if(frm.date_begin.value !="" && frm.date_begin.value!="0000-00-00"){
	 ...
	 }else if(frm.date_begin.value !="" && frm.date_begin.value=="0000-00-00"){
	 	frm.date_begin.value=frm.date_begin.value+"(--)";	   
	 }
	 if(frm.date_end.value !="" && frm.date_end.value!="0000-00-00"){
	...
	}else if(frm.date_end.value !="" && frm.date_end.value=="0000-00-00"){
	 	frm.date_end.value=frm.date_end.value+"(--)";	   	
	 } 	 
//	 frm.kanri_no.value ="契-"+frm.kanri_no.value; 
	 	
      if ( confirm("登録しますか?") != 1 ) { return; }	
     	frm.action = "<%=urlPage%>tokubetu/admin/contract/add.jsp";	
	frm.submit(); 
   }   
...
function goLCategory(){	
	var param="";
	openScrollWin("<%=urlPage%>tokubetu/admin/contract/cateParent_pop.jsp", "categoryAdd", "categoryAdd", "350", "500",param);
} 
function goInit(){
...
<tr>
    <td style="padding: 0px 0px 2px 15px">    	
   		<font color="#807265">※管理番号： 自動的に付番されますが、もし特定番号を使う場合、<span class="calendar16_1">使っている番号</span>の使用は避けて下さい。それを参考して管理番号を決めて下さい。</font><br>
      	 	<font color="#807265">※契約期間 / 契約日：特定日付を指定しない場合は、[0000-00-00]--->このままでいいんです。</font>
    </td>     
...
<table width="950"  class="tablebox" cellspacing="5" cellpadding="5" >				
	<form name="kubun" method="post" >													
		<tr>
			<td width="12%"><font color="#CC0000">※</font><span class="titlename">契約区分選択</span></td>
			<td width="88%"  colspan="3">
				<select name="kubun_noVal"  onChange="return codeView();">					
...
<%
int i=1;
Iterator listiter=listKubun.iterator();
	while (listiter.hasNext()){				
		Category dbb=(Category)listiter.next();
		int seq=dbb.getBseq();																				
%>							
				<%if(kubun_bseqint==seq){%><option  value="<%=seq%>"  selected><%=dbb.getName()%></option><%}%>
				<%if(kubun_bseqint!=seq){%><option  value="<%=seq%>"  ><%=dbb.getName()%></option> <%}%>				
<%
i++;	
}
%>				
			...
			<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value=" :::契約区分編集::: " onClick="goLCategory();">	       	
			</td>						
		</tr>
	</form>
</table>
<div class="clear_margin"></div>					
<table width="950"  class="tablebox" cellspacing="5" cellpadding="5" >				
	<form name="forminput" method=post  action="<%=urlPage%>tokubetu/admin/contract/add.jsp" enctype="multipart/form-data">		
		...
		<input type='hidden' name="kubun_bseq" value="<%=kubun_bseq%>">												
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">契約区分</span></td>
				<td width="88%"  colspan="3">											
					<input type="text" maxlength="30" name="kubunCate" value="<%=catenm%>" class="input02" style="width:100px" readonly>
				</td>				
			</tr>
			<tr >								
				<td width="12%"><img src="<%=urlPage%>rms/image/icon_s.gif" ><span class="titlename">管理番号参考</span></td>
...
<table width="950"   class="tablebox" cellspacing="2" cellpadding="2">													
			<tr>
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">契約形態</span></td>
...
<table width="950"   class="tablebox" cellspacing="5" cellpadding="5">			
	<tr>	
		<td  width="12%"><img src="<%=urlPage%>rms/image/icon_s.gif" ><span class="titlename">契約書</span></td>
		<td width="88%" ><input type="file" size="80"  name="fileNm" class="file_solid"><font color="#807265" >(▷ '&,%,^'などの記号は使わないで下さい!)</font></td>		
	</tr>
	<tr>	
