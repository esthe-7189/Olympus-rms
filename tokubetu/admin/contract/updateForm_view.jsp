...
int mseq=0; int levelVal=0;String title=""; 
...
String con[]=kanri_no.split("-");              
 ...
 ContractBeen codeItem=mana.getCodeLimit(kubun);
 
 List listCode=mana.listCode(kubun); 
...
//	frm.kanri_no.value ="契-"+frm.kanri_no.value; 
	...
	frm.sekining_nm.value =frm.sekiningNm.options[frm.sekiningNm.selectedIndex].text;	  
	 ...
	 if(frm.date_begin.value !="" && frm.date_begin.value.substring(0,10)!="0000-00-00"){
	 ...
	 }else if(frm.date_begin.value !="" && frm.date_begin.value.substring(0,10)=="0000-00-00"){
	 	frm.date_begin.value=frm.date_begin.value.substring(0,10)+"(--)";	   	
	 }

	 if(frm.date_end.value !=""  && frm.date_end.value.substring(0,10) !="0000-00-00"){
	 ...
	 }else if(frm.date_end.value !="" && frm.date_end.value.substring(0,10)=="0000-00-00"){
	 	frm.date_end.value=frm.date_end.value.substring(0,10)+"(--)";	   
	 } 	 

      if ( confirm("修正しますか?") != 1 ) { return; }	
     	frm.action = "<%=urlPage%>tokubetu/admin/contract/update.jsp";	
	frm.submit(); 
   }   
...
<table width="950"  class="tablebox" cellspacing="5" cellpadding="5" >				
	<form name="forminput" method=post  action="<%=urlPage%>tokubetu/admin/contract/update.jsp" enctype="multipart/form-data">				
		...
		<input type='hidden' name="kubun_bseq" value="<%=kubun%>">												
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">契約区分</span></td>
				<td width="88%"  colspan="3"><%=board.getName()%></td>				
			</tr>
			<tr >								
...
<c:if test="${! empty  listFollow}">
	<c:forEach var="mem" items="${listFollow}"  varStatus="idx"  >		
			...
			<tr >								
				...
				<td>	<span class="calendar15">契 - </span>
