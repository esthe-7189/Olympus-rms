...
function formSubmit(){        
	...
	with (document.forminput) {
	  ...
	  if(frm.date_submit.value !="" && frm.date_submit.value.substring(0,10)!="0000-00-00"){
	 ...
	 }else if(frm.date_submit.value !="" && frm.date_submit.value.substring(0,10)=="0000-00-00"){
	 	frm.date_submit.value=frm.date_submit.value.substring(0,10)+"(--)";	   	
	 }
	 if(frm.date_sign.value !="" && frm.date_sign.value.substring(0,10) !="0000-00-00"){
	 ...
	 }else if(frm.date_sign.value !="" && frm.date_sign.value.substring(0,10)=="0000-00-00"){
	 	frm.date_sign.value=frm.date_sign.value.substring(0,10)+"(--)";	   	
	 } 		
      if ( confirm("修正しますか?") != 1 ) { return; }	
     	frm.action = "<%=urlPage%>tokubetu/admin/approval/update.jsp";	
	frm.submit(); 
   }   
...
<tr>
    <td style="padding: 0px 0px 2px 15px">    	
   		<font color="#807265">※管理番号： <span class="calendar16_1">使っている番号</span>の使用は避けて下さい。それを参考して管理番号を決めて下さい。</font><br>
...
<table width="950"  class="tablebox" cellspacing="3" cellpadding="3" >				
	<form name="forminput" method=post  action="<%=urlPage%>tokubetu/admin/approval/add.jsp" enctype="multipart/form-data">		
		...
		<input type='hidden' name="gian_mseq" value="">											
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">お名前</span></td>
				<td width="88%"  colspan="3"><%=nmMseq%></td>
			</tr>
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">管理No</span></td>
				<td width="88%"  colspan="3"><span class="calendar16_1"> 使っている番号:</span>
				<select name="codeVal" >
			...
			<c:if test="${! empty listCode}">					
				<c:forEach var="code" items="${listCode}" varStatus="index" >					
					...
					<input type="text" maxlength="30" name="kanri_no" value="${board.kanri_no}" class="input02" style="width:100px"><font color="#807265" >(▷例：2013年の場合==>  決13-000)</font>					
				</td>				
			</tr>
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">決裁項目</span></td>
				<td width="50%">
			...
			<c:if test="${! empty listCate}">						
				...
				<td width="10%"><font color="#CC0000">※</font><span class="titlename">起案者</span></td>
				<td width="28%">
