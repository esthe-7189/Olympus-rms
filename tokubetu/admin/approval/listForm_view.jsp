...
<div id="searchBox">
	<div class="searchBox_left">
		<form name="searchCate"  action="<%=urlPage%>tokubetu/admin/approval/listForm.jsp" method="post">		
	...
	<form name="searchjitsi"  action="<%=urlPage%>tokubetu/admin/approval/listForm.jsp" method="post">			
...
<form name="search"  action="<%=urlPage%>tokubetu/admin/approval/listForm.jsp" method="post">
...
<div class="searchBox_all">				
	<img src="<%=urlPage%>rms/image/admin/location.gif" align="absmiddle"><span class="calendar9">決裁日検索</span>	
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" >
			<form name="searchDate"   action="<%=urlPage%>tokubetu/admin/approval/listForm.jsp" method="post"  >
				<tr >
...
<!-- ************************page No end***********************************-->		
			...
			<td width="10%" align="right"><font  color="#FF6600">(<%=count%>個)</font></td>
...
%>								
	...
	<tr height=25 onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">	    
	    ...
	    <td align="center" class="line_gray_bottomnright">			
	    		<%if(db.getMseq()==mseq || id.equals("funakubo") || id.equals("juc0318") || id.equals("admin")){%>
...
function goPage(pageNo) {    
	...
	document.move.action = "<%=urlPage%>tokubetu/admin/approval/listForm.jsp";
    	document.move.submit();
}
function goDelete(code,seq,filename) {
	if(confirm(code+"のデータを削除しますか?")!=1){return;}
	document.move.action = "<%=urlPage%>tokubetu/admin/approval/delete.jsp";
	document.move.seq.value = seq;	
	document.move.filename.value = filename;
    	document.move.submit();
}
function goModify(seq) {
    	document.move.action = "<%=urlPage%>tokubetu/admin/approval/updateForm.jsp";
	document.move.seq.value=seq;
...
function goDown(seq,filename) {			
	document.move.action = "<%=urlPage%>tokubetu/admin/approval/down.jsp";
	document.move.seq.value = seq;	
	document.move.filename.value = filename;
	document.move.submit();
}
function excelDown(){			
	document.move.action = "<%=urlPage%>tokubetu/admin/approval/listExcel.jsp";	
    	document.move.submit();    	
}
...
function excelExplain(){		
	openNoScrollWin("<%=urlPage%>tokubetu/admin/approval/excelDown_pop.jsp", "決裁書", "決裁書", "760", "525","");
}
function printDown(){			
	openScrollWin("<%=urlPage%>tokubetu/admin/approval/print_pop.jsp", "決裁書", "決裁書", "760", "700","");	
}
/*
function printDown(){		
	...
	openScrollWin("<%=urlPage%>rms/admin/kintai/printFormAll.jsp", "決裁書", "決裁書", "500", "700",param);	
}
*/
</script>
