...
String yyVal=""; int yyValint=0;

String kodosi=inDate.substring(2,4);
int	kodosiint=Integer.parseInt(kodosi);

String kanri_yy=request.getParameter("kanri_yy");
if(kanri_yy==null){ 
	yyVal=inDate.substring(2,4);
 	yyValint=Integer.parseInt(yyVal);
}else{
	yyVal=kanri_yy;
 	yyValint=Integer.parseInt(yyVal);
}
...
Member member=managermem.getMember(id);
 ...
 ApprovalBeen codeItem=mana.getCodeLimit(yyVal);
...
function formSubmit(){        
	 ...
	 frm.kanri_no.value="決"+frm.year_sch.value+"-"+frm.year_str.value;	 	 
	 ...
	 if(frm.fileNm.value==""){
	  ...
	  if(frm.date_submit.value !="" && frm.date_submit.value!="0000-00-00"){
	 ...
	 }else if(frm.date_submit.value !="" && frm.date_submit.value=="0000-00-00"){
	 	frm.date_submit.value=frm.date_submit.value+"(--)";	   	
	 }
	 if(frm.date_sign.value !="" && frm.date_sign.value!="0000-00-00"){
	 ...
	 }else if(frm.date_sign.value !="" && frm.date_sign.value=="0000-00-00"){
	 	frm.date_sign.value=frm.date_sign.value+"(--)";	   	
	 } 	
	 
	 	
      if ( confirm("登録しますか?") != 1 ) { return; }	
     	frm.action = "<%=urlPage%>tokubetu/admin/approval/add.jsp";	
	frm.submit(); 
   }   
...
<div id="botton_position">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value=" 全体目録 " onClick="location.href='<%=urlPage%>tokubetu/admin/approval/listForm.jsp'">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value=" 新規登録 " onClick="location.href='<%=urlPage%>tokubetu/admin/approval/addForm.jsp'">			
</div>
...
<tr>
    <td style="padding: 0px 0px 2px 15px">    	
   		<font color="#807265">※管理番号： 自動的に付番されますが、もし特定番号を使う場合、<span class="calendar16_1">使っている番号</span>の使用は避けて下さい。それを参考して管理番号を決めて下さい。</font><br>
      	 	<font color="#807265">※受付日 / 決裁日： 特定日付を指定しない場合は、[0000-00-00]--->このままでいいんです。</font>
    </td>     
...
<table width="950"  class="tablebox" cellspacing="3" cellpadding="3" >				
	<form name="forminput" method=post  action="<%=urlPage%>tokubetu/admin/approval/add.jsp" enctype="multipart/form-data">		
		...
		<input type="hidden" name="kanri_no" value="">											
			<tr >								
				...
				<td width="88%"  class="calendar16_1" colspan="3">				
					    使っている番号:
				<select name="codeVal" >
			...
			<tr >								
				<td width="12%"><font color="#CC0000">※</font><span class="titlename">管理番号</span></td>
				<td width="50%" >				
					   <span class="calendar16_1">決</span>								   
				  <select name="year_sch" class="select_type3"  onChange="return goYear();">				  	  
				<%	for(int i=11;i<=kodosiint+2;i++){%>
					<%if(i==yyValint){%>
					...
					<%}%>
				<%}%>																			
				  </select>-<input type="text" maxlength="30" name="year_str" value="<%=conCode%>" class="input02" style="width:100px">					
					<font color="#807265" >(▷例：2013年の場合==>  決13-000)</font>
				</td>
				<td width="10%"><font color="#CC0000">※</font><span class="titlename">お名前</span></td>
				<td width="28%"><%=wname%></td>
			</tr>
			<tr >								
...
<table width="950"   class="tablebox" cellspacing="5" cellpadding="5">				
	<tr>	
		<td width="12%"><img src="<%=urlPage%>rms/image/icon_s.gif" ><span class="titlename">決裁書</span></td>
		<td width="88%" colspan="3"><input type="file" size="80"  name="fileNm" class="file_solid"> <font color="#807265" >(▷ '&,%,^'などの記号は使わないで下さい!)</font></td>		
	</tr>
	<tr>	
...
<form name="move"  method="post">
	...
	<input type="hidden" name="kanri_yy"  value="">
</form>			

<script language="JavaScript">
function goYear(){
	document.move.kanri_yy.value=document.forminput.year_sch.value;
	document.move.action = "<%=urlPage%>tokubetu/admin/approval/addForm.jsp";	
