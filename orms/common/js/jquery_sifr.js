...
(function (jQuery) {
	jQuery.fn.sifr = function (prefs) {
		...
		this.each(function () {
			var ele = jQuery(this), txt, alt, fir, embedOptions;
			...
			alt = ele.append('<span class="sIFR-jquery" style="position: absolute; ">' + jQuery.trim(fir.text()) + '</span>').children('.sIFR-jquery');
			txt = jQuery.trim(fir.html()).replace(/(>)\s+|\s+(<)/g, '$1$2').replace(/(id|name)=[A-Za-z0-9]+/g, '');
			...
			jQuery().flash(jQuery.extend(embedOptions, p.embedOptions), jQuery.extend({
				...
				alt.remove();
				ele.prepend(jQuery.fn.flash.transform(options));
			});
	...
	jQuery.sifr = function (prefs) {
		jQuery(document).sifr(jQuery.extend({
			save: true
		}, prefs));
	};
