...
<form name="move" method="post">
	...
	<tr>
	<form name="search"  action="<%=urlPage%>admin/job/jobList.jsp?page=1" method="post">		
		<td valign="middle" width="17%">			
		 ...
		 <td valign="middle" width="10%" align="left">
		 	<input type="button" name="" value="全体目録" onclick="location.href='<%=urlPage%>admin/job/jobList.jsp?page=1'" id="LIST!"  title="LIST!" class="button buttonBright" />		  
		 </td>
...
<%int i=1;	%>		
	...
	<%Iterator listiter=list.iterator();					
		while (listiter.hasNext()){
			JobBean job=(JobBean)listiter.next();
			int seq=job.getSeq();											
			if(seq!=0){	
				String yymmdd=dateFormat.format(job.getRegister());
				int view_yn=job.getView_yn();
				int cateL=	job.getCate_L_seq();
				int cateM=job.getCate_M_seq();			
	%>
			 <tr  onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">				 
				 <td><%=job.getView_seq()%></td>
				 <td align="left">
				...
				<td  align="left">
					<a href="javascript:goRead('<%=job.getSeq()%>')"  onfocus="this.blur()">
					<%=job.getTitle()%></a>
				</td>
				<td><%=yymmdd%></td>
				<td  align="center">
				...
				<td>
					<a href="javascript:goModify('<%=job.getSeq()%>')" onfocus="this.blur()">
					<img src="<%=urlPage%>images/admin/icon_admin_pen.gif" alt="Modify" >
					</a></td>
				<td>
					<a href="javascript:goDelete('<%=job.getSeq()%>')"  onfocus="this.blur()">
...
function goPage(pageNo) {
    document.move.action = "<%=urlPage%>admin/job/jobList.jsp";
    document.move.page.value = pageNo;
    document.move.submit();
}
function goRead(seq) {
    document.move.action = "<%=urlPage%>admin/job/readForm.jsp";
    document.move.seq.value = seq;
    document.move.submit();
}
function goModify(seq) {
	document.move.action = "<%=urlPage%>admin/job/updateForm.jsp";
	document.move.seq.value=seq;
    	document.move.submit();
}
function goDelete(seq) {
	...
	if ( confirm("本内容を削除しましょうか?") != 1 ) {
    	...
    	document.move.action = "<%=urlPage%>admin/job/deleteForm.jsp";	
    	document.move.submit();
}
