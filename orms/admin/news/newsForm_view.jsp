...
<table width="100%" border="0" cellspacing="0" cellpadding="0" valign="top">			
	<tr>		
    		<td align="left" width="100%"  style="padding: 10 0 0 10"  class="calendar15">
				...
				<img src="<%=urlPage%>images/common/ArrowNews.gif" style="filter:Alpha(Opacity=30);">ニュース管理
    		</td>    		
	</tr>	
	<tr>		
...
<form name="move" method="post">
	...
	<tr>
	<form name="search"  action="<%=urlPage%>admin/news/newsForm.jsp?page=1" method="post">		
		<td valign="middle" width="17%">			
		 ...
		 <td valign="middle" width="10%" align="left">
		 	<input type="button" name="" value="全体目録" onclick="location.href='<%=urlPage%>admin/news/newsForm.jsp?page=1'" id="LIST!"  title="LIST!" class="button buttonBright" />		  
		 </td>
		...
		<table width="95%" border=1 cellpadding=1 cellspacing=0 bordercolor=#FFFFFF bordercolorlight=#A2A2A2>
			<tr bgcolor="" align=center height=26>
				...
				<td   width="10%" bgcolor=#F1F1F1>修正</td>
				<td   width="10%" bgcolor=#F1F1F1>削除</td>				
			</tr>			
			...
			<tr  onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">				
				<td><fmt:formatDate value="${news.register}" pattern="yyyy-MM-dd" /></td>				
				<td  align="left">
					<a href="javascript:goRead('${news.seq}')"  onfocus="this.blur()">
					${news.title}</a>
				</td>					
				<c:if test="${news.view_yn==1}"><td  align="center">はい</td></c:if>
				<c:if test="${news.view_yn==2}"><td  align="center">いいえ</td></c:if>				
				<td>
...
<c:if test="<%= searchCondView_ok || searchCondView_no || searchCondTitle %>">
	検索条件: [
	<c:if test="<%= searchCondView_ok %>">展示中</c:if>
	<c:if test="<%= searchCondView_no %>">展示してない</c:if>	
