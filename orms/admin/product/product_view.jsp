...
String pageNum = request.getParameter("page");	
	...
	boolean searchCondCate01 = false;
	boolean searchCondCate02 = false;
	boolean searchCondCate03 = false;
	boolean searchCondTitle_m = false;	
	...
	if (searchCond != null && searchCond.length > 0 && searchKey != null){	
		...
		for (int i=0;i<searchCond.length ;i++ ){
			if (searchCond[i].equals("cate01")){
				whereCond.add("cate=1");				
				searchCondCate01=true;
			}else if (searchCond[i].equals("cate02")){
				whereCond.add("cate=2");		
				searchCondCate02 = true;	
			}else if (searchCond[i].equals("cate03")){
				whereCond.add("cate=3");		
				searchCondCate03 = true;			
			}else if (searchCond[i].equals("title_m")){
				whereCond.add("title_m LIKE '%"+searchKey+"%'");		
				searchCondTitle_m = true;
			}	
		}
	}
...
<form name="move" method="post">
    ...
    <c:if test="<%= searchCondCate01 %>">
    <input type="hidden" name="search_cond" value="cate01">
    </c:if>
    <c:if test="<%= searchCondCate02 %>">
    <input type="hidden" name="search_cond" value="cate02">
    </c:if>
    <c:if test="<%= searchCondCate03 %>">
    <input type="hidden" name="search_cond" value="cate03">
    </c:if>
    <c:if test="<%= searchCondTitle_m%>">
    <input type="hidden" name="search_cond" value="title_m">
    </c:if>	
	...
	<tr>
	<form name="search"  action="<%=urlPage%>admin/product/product.jsp?page=1" method="post">		
		<td valign="middle" width="17%">			
			<div  class="f_left" style="padding:2px;background:#EFEBE0;margin:0px 2px 0 0;">
				  <select name="search_cond" class="select_type2" onKeyPress="return doSubmitOnEnter()">
			            	<option name="search_cond" VALUE="cate01"  >自家軟骨細胞</OPTION>
			          	<option name="search_cond" VALUE="cate02"  >コラーゲン(Collagen)</OPTION>
			          	<option name="search_cond" VALUE="cate03"  >医療機器</OPTION>
					<option name="search_cond" VALUE="title_m"  >メイン　タイトル</OPTION>		
				  </select>
		        </div>
		</td>		        					
		 ...
		 <td valign="middle" width="10%" align="left">
		 	<input type="button" name="" value="全体目録" onclick="location.href='<%=urlPage%>admin/product/product.jsp?page=1'" id="LIST!"  title="LIST!" class="button buttonBright" />		  
		 </td>
		...
		<table width="95%" border=1 cellpadding=1 cellspacing=0 bordercolor=#FFFFFF bordercolorlight=#A2A2A2>
			<tr bgcolor="" align=center height=26>				
				<td  width="10%"  bgcolor=#F1F1F1>コード</td>
				<td  width="15%"  bgcolor=#F1F1F1>区分</td>
				<td  width="45%"  bgcolor=#F1F1F1>メイン　タイトル</td>						
				<td  width="12%" bgcolor=#F1F1F1>登録日</td>				
			...
			<tr  onMouseOver=this.style.backgroundColor="#EFF5F9" onMouseOut=this.style.backgroundColor="">				
				...
				<td>
					...
					<c:if test="${product.category==2}">コラーゲン(Collagen)</c:if>
					<c:if test="${product.category==3}">医療機器</c:if>
				</td>
				<td  align="left">
					<a href="javascript:goRead('${product.pseq}')"  onfocus="this.blur()">
					${product.title_m}</a>
				</td>				
				<td><fmt:formatDate value="${product.register}" pattern="yyyy-MM-dd" /></td>				
				<c:if test="${product.view_yn==1}"><td  align="center">はい</td></c:if>
				<c:if test="${product.view_yn==2}"><td  align="center">いいえ</td></c:if>				
				<td>
					<a href="javascript:goModify(${product.pseq})" onfocus="this.blur()">
					<img src="<%=urlPage%>images/admin/icon_admin_pen.gif" alt="Modify" >
					</a></td>
				<td>
					<a href="javascript:goDelete('${product.pseq}')"  onfocus="this.blur()">
...
function goPage(pageNo) {
    document.move.action = "<%=urlPage%>admin/product/product.jsp";
    document.move.page.value = pageNo;
    document.move.submit();
}
function goRead(pseq) {
    document.move.action = "<%=urlPage%>admin/product/read.jsp";
    document.move.pseq.value = pseq;
    document.move.submit();
}
function goModify(pseq) {
	document.move.action = "<%=urlPage%>admin/product/updateForm.jsp";
	document.move.pseq.value=pseq;
    	document.move.submit();
}
function goDelete(pseq) {
	document.move.pseq.value=pseq;	
	
	if ( confirm("本内容を削除しましょうか?") != 1 ) {
		return;
	}
    	document.move.action = "<%=urlPage%>admin/product/deleteForm.jsp";	
    	document.move.submit();
}
...
<tr>
	<td valign="middle" >	
<c:if test="<%= searchCondCate01 || searchCondCate02 || searchCondCate03 || searchCondTitle_m %>">
	検索条件: [
	<c:if test="<%= searchCondCate01 %>">自家軟骨細胞</c:if>
	<c:if test="<%= searchCondCate02 %>">コラーゲン(Collagen)</c:if>
	<c:if test="<%= searchCondCate03 %>">医療機器</c:if>
	<c:if test="<%= searchCondTitle_m %>">メイン　タイトル</c:if>	
	= 
	<%=searchKey%> ]
