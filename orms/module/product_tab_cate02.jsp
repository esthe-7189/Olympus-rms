...
String cate="2";
	...
	int count2 = manager.countUser(whereCond, whereValue);
	int totalPageCount2 = 0; //전체 페이지 개수를 저장
	int startRow=0, endRow=0;
	if (count2>0){
		totalPageCount2=count2/PAGE_SIZE;
		if (count2 % PAGE_SIZE > 0)totalPageCount2++;
		
		startRow=(currentPage-1)*PAGE_SIZE+1;
		endRow=currentPage*PAGE_SIZE;
		if(endRow > count2) endRow = count2;
	}
	if(count2<=0){
		startRow=startRow-0;
		endRow=endRow-0;
			...
			<c:if test="${! empty list}">
				<c:forEach var="product" items="${list}">
					<c:if test="${!empty product.pimg}">					    						
								<ul>						
									<li class="s_img">										
										<a href="javascript:goView('${product.pseq}')">
									...
									<li class="con">
										<ol >
											<li class="title"><a href="javascript:goView('${product.pseq}')">${product.title_m}</a></li>												
											<li class="content">${product.title_s}</li>
											<li class="date"><fmt:formatDate value="${product.register}" pattern="yyyy-MM-dd" /></li>
											<li class="more"><a href="javascript:goView('${product.pseq}')">read more</a></li>							
										</ol>
									</li>
								</ul>
					</c:if>	
...
function goView(pseq) {
    document.move02.action = "<%=urlPage%>product/item_product.jsp";    
    document.move02.pseq.value = pseq;
    document.move02.submit();
}
