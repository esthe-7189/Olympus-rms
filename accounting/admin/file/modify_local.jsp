...
if (frmFileItem.getSize() >0){
	...
	//파일을 지정한 경로에 저장
	File fileNmVal=new File( "C:\\dev\\tomcat5\\webapps\\orms\\accounting/admin/file/fileList",fil);
	
	//같은 이름의 파일 처리
	if (fileNmVal.exists())	{
		for (int i=0;true ;i++ ){
			fileNmVal=new File("C:\\dev\\tomcat5\\webapps\\orms\\accounting/admin/file/fileList","("+i+")"+fil);
			if (!fileNmVal.exists()){
				fil="("+i+")"+fil;
				break;
			}
