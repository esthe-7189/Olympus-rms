...
<div id="botton_position">	
	<input type="button"  class="cc" onfocus="this.blur();" style=cursor:pointer value="会員登録 >>" onClick="location.href='<%=urlPage%>accounting/admin/member/writeForm.jsp'">	
...
<div id="boxNoLineBig"  >		
	<table width="960">
 	<form name="search"  action="<%=urlPage%>accounting/admin/member/listForm.jsp" method="post">				
	<tr height="29">
		<td colspan="2">				
	...
	<tr>
		<td width="90%">	
			...
			<input type="button"  align=absmiddle value="  目録  >>"  style=cursor:pointer class="cc" onfocus="this.blur();"  onClick="location.href='<%=urlPage%>accounting/admin/member/listForm.jsp'">		
		</td>
...
<form name="frm" >    
		<tr height=30 bgcolor=#F1F1F1 align=center>				
			...
			<td class="title_list_m"><font  color="#FF0FFF">Level</font></td>
		...
		<%}%>)
			...
			<%}%>
					...
					<td width="5%"  align="center">						
						...
						<a href="<%=urlPage%>accounting/admin/member/updateForm_admin.jsp?member_id=<%=member.getMember_id()%>"  onfocus="this.blur()">
						<img src="<%=urlPage%>rms/image/admin/btn_cate_pen.gif"  align="absmiddle" alt="書き直し"></a>
					</td>										
					<td width="5%"  align="center">						
						<a href="<%=urlPage%>accounting/admin/member/delete.jsp?member_id=<%=member.getMember_id()%>&mseq=<%=member.getMseq()%>"  onfocus="this.blur()">
				...
				<td  style="padding: 5px 5px 5px 5px;"  colspan="11" align="center" width="90%" >
					<span id="itemData_block<%=i%>" style="DISPLAY:none; xCURSOR:hand;background:#EBEBD8;padding:7px 0px 7px 0px;">												
									<table width="70%"  border=1 cellpadding=1 cellspacing=0 bordercolor=#FFFFFF bordercolorlight=#A2A2A2 >
										<tr>	
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">住所:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">						
												<font  color="#6699CC">(<%=member.getZip()%>)</font>) <%=member.getAddress()%>
											</td>
										</tr>	
										<tr>	
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">お誕生日:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">						
												<%=member.getBir_day()%>
											</td>
										</tr>	
						　　　　　　　　　<tr>	
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">職名:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">						
												<%=member.getPosition()%>
											</td>
										</tr>	
										<tr>	
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">秘密番号(質問/答え):</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">						
										...
										<tr>	
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">ip番号:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">						
												<%=member.getIp_info()%>	
											</td>
										</tr>	
										<tr>
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">承認可否変更:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">	
								...
								<%}%>								
										...
										<tr>
											<td style="padding: 5px 5px 5px 5px" bgcolor=#F1F1F1 width="20%" align="center">Level変更:</td>							
											<td style="padding: 5px 5px 5px 5px" bgcolor=#FFFFFF width="80%">	
								<% if(memId.equals("admin")){%>
													adminは変更できません！！
								<%}else{%>
...
function goPage(pageNo) {
    document.move.action = "<%=urlPage%>accounting/admin/member/listForm.jsp";
    document.move.page.value = pageNo;
    document.move.submit();
}
function goModiView(member_id) {
    document.move.action = "<%=urlPage%>accounting/admin/member/updateForm_admin.jsp";    
...
function goOkView(mseq) {		
	...
	document.move.action = "<%=urlPage%>accounting/admin/member/member_yn.jsp";
...
function goLevelView(mseq) {		
	...
	var valueMseq=(jinoValue[0].checked==true) ?  "1" : "2";		
	document.move.action = "<%=urlPage%>accounting/admin/member/member_level.jsp";
	document.move.mseq2.value = mseq;
	document.move.okYn.value = valueMseq;
	document.move.submit();
}
