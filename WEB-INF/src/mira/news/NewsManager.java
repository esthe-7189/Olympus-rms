...
public class NewsManager { 
    ...
    /** 새로운 글을 삽입한다.*/ 
        ...
        try {
            ...
            // 새로운 글의 번호를 구한다.
            news.setSeq(Sequencer.nextId(conn, "news"));
            // 글을 삽입한다.
...
* 제목과 내용만 변경한다. */ 
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);        
            conn.setAutoCommit(false);
            pstmtUpdate = conn.prepareStatement( "update news set title=?,content=?,view_yn=? where seq=?");
	...
	public int count(List whereCond, Map valueMap)   throws NewsManagerException {
        ...
        try {
			...
			query.append("select count(*) from news ");
    ...
    /** * 지정한 글을 읽어온다. */ 
        ...
        try {
            ...
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("select * from news where seq = ?");
            pstmt.setInt(1, seq); 
			rs = pstmt.executeQuery();
            if (rs.next()) {   
				news= new NewsBean();
...
public void delete(int seq) throws NewsManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("delete from news where seq = ?");
            pstmt.setInt(1, seq);
            pstmt.executeUpdate();
        } catch(SQLException ex) {
...
/**main special  **/
 public List selectMainSpecialNews(int pgcound) throws NewsManagerException {
        Connection conn = null;        
		PreparedStatement pstmt = null;
        ResultSet rs = null; 		 
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from news where view_yn=1 order by seq desc limit 0, ?" );
			pstmt.setInt(1, pgcound);			
			rs = pstmt.executeQuery();
            if (rs.next()) {   
            ...
            throw new NewsManagerException("select selectMainSpecial", ex);
        } finally {             
			if (rs != null)                  
				try { rs.close(); } catch(SQLException ex) {} 
            if (pstmt != null) 
