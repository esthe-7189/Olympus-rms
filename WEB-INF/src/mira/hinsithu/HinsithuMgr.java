...
public class HinsithuMgr {
	private static HinsithuMgr instance = new HinsithuMgr();
	
	public static HinsithuMgr getInstance() {		
		return instance;
	}
	...
	// level 1 인서트
	public void insertLevel_1(HinsithuBean pds)  throws MgrException {		
		...
		try {
			...
			conn.setAutoCommit(false);
			 // 새로운 글의 번호를 구한다.
            pds.setNo(Sequencer.nextId(conn, "hinsithu_qa"));
			// DB에 내용 저장				
			pstmt = conn.prepareStatement("insert into hinsithu_qa  values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
// level 2~4 인서트
public void insertLevel_2(HinsithuBean pds)  throws MgrException {		
		Connection conn = null;
		PreparedStatement pstmt = null;		
		PreparedStatement pstmt2 = null;		
		...
		try {
			...
			conn.setAutoCommit(false);
			 // 새로운 글의 번호를 구한다.
            pds.setBseq(Sequencer.nextId(conn, "hinsithu_kind"));
			// DB에 내용 저장				
			pstmt = conn.prepareStatement("insert into hinsithu_kind  values (?,?,?,?,?,?,?,?)");
...
public void updateHenkoLeve_1(int no, int  kind_yn) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("update hinsithu_qa set  kind_yn=?   where  fno=? ");
...
public void updateHenkoLeve_2(int no,  int  kind_yn) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("update hinsithu_kind set  kind_yn=?   where  bseq=? ");
		pstmt.setInt(1, kind_yn);		
		pstmt.setInt(2, no);		
		pstmt.executeUpdate();		
	}catch (SQLException ex){
...
/**수정하기**/
	public void update(HinsithuBean pds) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	try{
		conn=DBUtil.getConnection(POOLNAME);
		conn.setAutoCommit(false);
		pstmt=conn.prepareStatement("update hinsithu_qa set  file_kind=?,filename=?,cate_code=?,cate_code_det=?,fname=?, "+
...
/**수정하기2**/
	public void update2(HinsithuBean pds) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	try{
		conn=DBUtil.getConnection(POOLNAME);
		conn.setAutoCommit(false);
		pstmt=conn.prepareStatement("update hinsithu_qa set  file_kind=?,cate_code=?,cate_code_det=?,fname=?, "+
			"fname_digi=?,fname_bun=?,basho=?,basho_digi=?,basho_bun=?,fcomment=?,view_yn=?,f_title=?,cate_code_s=?  where  fno=?" );
		pstmt.setInt(1, pds.getFile_kind());
...
/**수정하기pop**/
	public void updateLevelall(HinsithuBean pds) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	try{
		conn=DBUtil.getConnection(POOLNAME);
		conn.setAutoCommit(false);
		pstmt=conn.prepareStatement("update hinsithu_kind set  filename=?,view_yn=?   where  bseq=?" );
...
/**수정하기pop02**/
	public void updateLevel(HinsithuBean pds) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	try{
		conn=DBUtil.getConnection(POOLNAME);
		conn.setAutoCommit(false);
		pstmt=conn.prepareStatement("update hinsithu_kind set  view_yn=?   where  bseq=?" );		
...
public int count(List whereCond, Map valueMap)   throws MgrException {
        ...
        try {
			...
			query.append("select count(*) from hinsithu_qa  where  file_kind=1 ");
...
public List selectList(List whereCond, Map valueMap, int startRow, int endRow) throws MgrException{
	...
	ResultSet rs=null;
	 try {
            StringBuffer query = new StringBuffer(200);             
			query.append("select * from hinsithu_qa  where  file_kind=1 ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append(" and ");                
				for (int i = 0 ; i < whereCond.size() ; i++) {
                       query.append(whereCond.get(i));                     
		...
		if (rs.next()){
			...
			Reader reader = null;        
			do{
				HinsithuBean pds=new HinsithuBean();
...
public List selectListAll() throws MgrException {
        ...
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from hinsithu_qa where  file_kind=1 order by fno desc " );
           	rs = pstmt.executeQuery();
            if (rs.next()) {   
            ...
            throw new MgrException("select seizo_list_cate", ex);
        } finally {             
			if (rs != null)                  
...
/**지정한 글 불러오기**/
public HinsithuBean getSeizo(int no) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	Reader reader = null;        
	try{
		HinsithuBean pds=null;
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("select * from hinsithu_qa where fno=?");
		pstmt.setInt(1, no);
		rs=pstmt.executeQuery();
		if (rs.next()){
			pds=new HinsithuBean();
				pds.setNo(rs.getInt("fno"));
				pds.setFile_kind(rs.getInt("file_kind"));
				pds.setFilename(rs.getString("filename"));
				pds.setCate_code(rs.getString("cate_code"));
...
//level 2이상 파일
public HinsithuBean getFile(int bseq) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	Reader reader = null;        
	try{
		HinsithuBean pds=null;
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("select * from hinsithu_kind where bseq=?");
		pstmt.setInt(1, bseq);
		rs=pstmt.executeQuery();
		if (rs.next()){
			pds=new HinsithuBean();
...
/**지정한 단계 불러오기**/
public HinsithuBean getKind2( int no, int  kind) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	try{
		HinsithuBean pds=null;
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("select * from hinsithu_kind  where fno =? and  file_kind =? ");
		pstmt.setInt(1, no);
		pstmt.setInt(2, kind);
		rs=pstmt.executeQuery();
		if (rs.next()){
...
/**지정한 Level 불러오기**/
public HinsithuBean getLevel( int no) throws MgrException{
	Connection conn=null;
	PreparedStatement pstmt=null;
	ResultSet rs=null;
	try{
...
public void deleteLevel1(int no) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("delete from hinsithu_qa where fno=?  ");
		pstmt.setInt(1, no);
		pstmt.executeUpdate();		

		pstmtKind=conn.prepareStatement("delete from hinsithu_kind  where fno=?  ");
...
public void deleteLevel2(int no) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("delete from hinsithu_kind where   fno=? ");
		pstmt.setInt(1, no);
		pstmt.executeUpdate();		
...
public void deleteLevel3(int no) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("delete from hinsithu_kind where bseq=?  or  parent_id =? ");
		pstmt.setInt(1, no);
		pstmt.setInt(2, no);
		pstmt.executeUpdate();		
...
public void deleteLevel4(int no) throws MgrException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("delete from hinsithu_kind where bseq=?   ");
		pstmt.setInt(1, no);
		pstmt.executeUpdate();		
...
public void upDown(int no)	throws MgrException {		
		...
		try {
			conn = DBUtil.getConnection(POOLNAME); 			
			stmt = conn.prepareStatement("update hinsithu_qa set read_no = read_no+1 where fno=?");			
			stmt.setInt(1,no);
			stmt.executeUpdate();
			
		} catch(SQLException ex) {
			throw new MgrException("hinsithu_qa upDown",ex);
		}  finally {			
