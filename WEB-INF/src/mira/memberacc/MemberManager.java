 ...
 //*********************user***************************   
    public void insertMember(Member member)  throws MemberManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
			 ...
			 // 새로운 회원의 번호를 구한다.
            member.setMseq(Sequencer.nextId(conn,"memberacc"));		
            ...
            throw new MemberManagerException("insertMemberacc", ex);
        } finally {
    ...
    public Member getMember(String member_id)  throws MemberManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
            	"select * from memberacc where member_id = ?");
            ...
            throw new MemberManagerException("getMemberacc", ex);
        } finally {
    ...
    public void updateMember(Member member) throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            "update memberacc set  member_id=?, mail_address=?, tel=?,password=?,nm=?, "+
			"hurigana=?,zip=?,address=? ,sex=?,bir_day=?,position=?, himithu_1=?,himithu_2=?, member_yn=?, ip_info=?, level=?  "+			
			"where mseq=?");  
			...
			pstmt.setInt(16, member.getLevel());
            ...
            throw new MemberManagerException("updateMemberacc", ex);
        } finally {
            if (pstmt != null) try { pstmt.close(); } catch(SQLException ex) {}
            if (conn != null) try { conn.close(); } catch(SQLException ex) {}
        }
    ...
    public void deleteMember(int mseq)   throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"delete from memberacc where mseq=?");
            pstmt.setInt(1, mseq);
	 ...
	 public Member  delSelect(int mseq) throws MemberManagerException {
        ...
        try {
            ...
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("select * from memberacc  where mseq = ?");
            pstmt.setInt(1, mseq); 
			rs = pstmt.executeQuery();
            if (rs.next()) {   
				member= new Member();
    ...
    throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
                "select password from memberacc  where member_id = ?");
            pstmt.setString(1, member_id);
            rs = pstmt.executeQuery();
            if (rs.next()) {
	...
	public Member idCh(String member_id)   throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select  member_id  from memberacc  where member_id = ?");
            pstmt.setString(1, member_id);
...
//승인받은 회원인지 확인  member_yn=2일경우 승인됨
	public Member adminUse(String member_id)   throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"select  member_yn  from memberacc  where member_id = ?");
            pstmt.setString(1, member_id);
...
public Member idChk(String nm, String himithu_1,String himithu_2) throws MemberManagerException{
	...
	try{
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement(
			"select member_id,password  from memberacc where nm=? and himithu_1=? and himithu_2=?  ");
...
// 등록된 글의 개수를 구한다.    
	public int count(List whereCond, Map valueMap) throws MemberManagerException {
        ...
        try {
			...
			query.append("select count(*) from  memberacc ");
   ...
   /** * 목록을 읽어온다.  */     
    ...
    throws MemberManagerException {
        ...
        try {
            StringBuffer query = new StringBuffer(200);             
			query.append("select * from memberacc ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where ");                
				for (int i = 0 ; i < whereCond.size() ; i++) {
                       query.append(whereCond.get(i));                     
...
public void memberYn(int mseq, int member_yn) throws MemberManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement("update memberacc set member_yn=?  where mseq=?");            
...
public void levelChange(int mseq, int level) throws MemberManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement("update memberacc set level=?  where mseq=?");            
...
//comment 패스워드 체크
   public int checkPass(int mseq, String password) throws MemberManagerException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
                "select password from memberacc where mseq=?");
            pstmt.setInt(1, mseq);
            rs = pstmt.executeQuery();
            if (rs.next()) {
                String dbPassword = rs.getString("password");
