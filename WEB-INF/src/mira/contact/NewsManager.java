...
public class NewsManager { 
    ...
    /** 새로운 글을 삽입한다.*/ 
        ...
        try {
            ...
            // 새로운 글의 번호를 구한다.
            news.setSeq(Sequencer.nextId(conn, "contact"));
            // 글을 삽입한다.
...
* 제목과 내용만 변경한다. */ 
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);        
            conn.setAutoCommit(false);
            pstmtUpdate = conn.prepareStatement( "update contact set title=?,content=?,view_yn=? where seq=?");
    ...
    throws NewsManagerException {
        ...
        try {
			...
			query.append("select * from contact ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where "); 
    ...
    /** * 지정한 글을 읽어온다. */ 
        ...
        try {
            ...
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("select * from contact where seq = ?");
            pstmt.setInt(1, seq); 
			rs = pstmt.executeQuery();
            if (rs.next()) {   
				news= new NewsBean();
...
public void delete(int seq) throws NewsManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("delete from contact where seq = ?");
            pstmt.setInt(1, seq);
            pstmt.executeUpdate();
        } catch(SQLException ex) {
