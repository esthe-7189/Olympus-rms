...
public class NewsManager { 
    ...
    /** 새로운 글을 삽입한다.*/ 
        ...
        try {
            ...
            // 새로운 글의 번호를 구한다.
            news.setSeq(Sequencer.nextId(conn, "hitokoto"));
            // 글을 삽입한다.
            pstmtInsert = conn.prepareStatement( 
 "insert into hitokoto values (?,?,?,?,?,?)");
			...
			pstmtInsert.setString(6, news.getNm());
...
* 제목과 내용만 변경한다. */ 
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);        
            conn.setAutoCommit(false);
            pstmtUpdate = conn.prepareStatement( "update hitokoto set title=?,content=?,view_yn=? where seq=?");
	...
	public int count(List whereCond, Map valueMap)   throws NewsManagerException {
        ...
        try {
			...
			query.append("select count(*) from hitokoto ");
    ...
    throws NewsManagerException {
        ...
        try {
			...
			query.append("select * from hitokoto ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where "); 
    ...
    throws NewsManagerException {
        ...
        try {
			...
			query.append("select * from hitokoto where view_yn=1 ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("and "); 
    ...
    /** * 지정한 글을 읽어온다. */ 
        ...
        try {
            ...
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("select * from hitokoto where seq = ?");
            pstmt.setInt(1, seq); 
			rs = pstmt.executeQuery();
            if (rs.next()) {   
				news= new NewsBean();
...
public void delete(int seq) throws NewsManagerException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            pstmt = conn.prepareStatement("delete from hitokoto where seq = ?");
            pstmt.setInt(1, seq);
            pstmt.executeUpdate();
        } catch(SQLException ex) {
...
//답변해결 첸지
public void viewYn(int seq, int view_yn) throws NewsManagerException {
        Connection conn = null;
        PreparedStatement pstmt = null;        
        try {
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement("update hitokoto set view_yn=?  where seq=?");            
			pstmt.setInt(1, view_yn);
			pstmt.setInt(2, seq);		
            
            int result = pstmt.executeUpdate();
            if (result == 0) {
                throw new NewsManagerException("viewYn  !!");
            }
        } catch(SQLException ex) {
            throw new NewsManagerException("viewYn!!", ex);
        } finally {
            if (pstmt != null) try { pstmt.close(); } catch(SQLException ex) {}
            if (conn != null) try { conn.close(); } catch(SQLException ex) {}
        }
