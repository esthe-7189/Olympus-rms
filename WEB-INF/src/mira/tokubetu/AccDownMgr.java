	...
	public void insertAccDown(AccBean pds)  throws MgrException {		
		...
		try {
			...
			conn.setAutoCommit(false);
			 // 새로운 글의 번호를 구한다.
            pds.setSeq(Sequencer.nextId(conn, "toku_down"));
			// DB에 내용 저장				
			pstmt = conn.prepareStatement("insert into toku_down  values (?,?,?,?,?)");
...
public AccBean getDown(int seq) throws MgrException{
	...
	try{
		AccBean pds=null;
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("select * from toku_down where seq=?");
		pstmt.setInt(1, seq);
		rs=pstmt.executeQuery();
		if (rs.next()){
			pds=new AccBean();
				pds.setSeq(rs.getInt("seq"));
				pds.setSeq_acc(rs.getInt("seq_aa"));
	 ...
	 public List selectDownDtail(int seq_acc) throws MgrException {
        ...
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from toku_down a inner join membertoku b on a.seq_mem=b.mseq where seq_acc=? order by seq desc;" );
