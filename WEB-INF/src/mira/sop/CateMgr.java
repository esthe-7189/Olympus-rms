	...
	PreparedStatement pstmtInsertMessage = null;        
	  ...
	  try {
            ...
            if (board.getParentId() ==0) {                 
				...
				rsGroup = stmtGroup.executeQuery("select max(GROUP_ID) from sop_cate");                
					int maxGroupId = 0;                
					if (rsGroup.next()) {
				...
				pstmtOrder = conn.prepareStatement( "select max(ORDER_NO) from sop_cate "
            ...
            if (board.getOrderNo() > 0) {
                pstmtOrderUpdate = conn.prepareStatement(
					"update sop_cate set ORDER_NO = ORDER_NO + 1,CATE_NO = CATE_NO + 1 "	+
            ...
            board.setBseq(Sequencer.nextId(conn,"sop_cate"));	
            pstmtInsertMessage = conn.prepareStatement( "insert into sop_cate values (?,?,?,?,?,?,?)");
...
public void update(int id, String nm, int  cateno) throws MgrException {
        ...
        try {
            ...
            conn.setAutoCommit(false);
			 ...
			 pstmtMessage = conn.prepareStatement( "update sop_cate set NM=?, CATE_NO=?   where BSEQ=?");
	...
	public int count(List whereCond, Map valueMap)   throws MgrException {
        ...
        try {
			...
			query.append("select count(*) from sop_cate ");
    ...
    throws MgrException {
        ...
        try {
			...
			query.append("select * from sop_cate ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where "); 
 ...
 public Category select(int id) throws MgrException {
        ...
        try {
            ...
            pstmtMessage = conn.prepareStatement(
                "select * from sop_cate  where BSEQ = ?");
    ...
    public void delete(int id, int level) throws MgrException {
        ...
        try {
			...
			if(level==1){
				pstmtMessage = conn.prepareStatement(
                "delete from sop_cate where BSEQ = ? or  PARENT_ID=? ");
				pstmtMessage.setInt(1, id);
				pstmtMessage.setInt(2, id);
			}else if(level==2){
				pstmtMessage = conn.prepareStatement(
                "delete from sop_cate where BSEQ = ? ");
				pstmtMessage.setInt(1, id);
			}  
			pstmtMessage.executeUpdate();	
 ...
 public List selectListAdminLevel(int no, int level) throws MgrException {
        ...
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from sop_cate where PARENT_ID=?  and PLEVEL=?  order by CATE_NO asc" );
            pstmt.setInt(1, no);		
...
/**현재위치 대분류 리스트  **/
 public List selectStayL( int level) throws MgrException {
        ...
        try {            
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
				"select * from sop_cate where PLEVEL=?  order by GROUP_ID asc " );            
...
/**main special  **/
 public List selectMainSpecialInfo( int level, int pgcound) throws MgrException {
        ...
        try {            
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
				"select * from sop_cate where PLEVEL=?  order by GROUP_ID asc limit 0, ?" );           
			pstmt.setInt(1, level);	
...
/**현재위치 중소분류 리스트::   GROUP_ID=원부모번호 ,   level=(중분류는 2,소분류는 3)  **/
 public List selectStayMS(int parent_id, int level) throws MgrException {
        ...
        try {            
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
				"select * from sop_cate  where GROUP_ID=?  and PLEVEL=?  order by  ORDER_NO asc" );
...
/****************카테고리 아이템 카운트********************/
 ...
 public int  cateCnt(int  id, int level) throws MgrException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            stmt = conn.prepareStatement("select count(*) from sop_cate where PARENT_ID=?  and PLEVEL=?");
			stmt.setInt(1, id);	
			stmt.setInt(2, level);	
