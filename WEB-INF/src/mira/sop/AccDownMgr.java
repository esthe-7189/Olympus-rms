	...
	public void insertAccDown(AccBean pds)  throws MgrException {		
		...
		try {
			...
			conn.setAutoCommit(false);
			 // 새로운 글의 번호를 구한다.
            pds.setSeq(Sequencer.nextId(conn, "sop_down"));
			// DB에 내용 저장				
			pstmt = conn.prepareStatement("insert into sop_down  values (?,?,?,?,?,?)");
...
public AccBean getDown(int seq) throws MgrException{
	...
	try{
		AccBean pds=null;
		conn=DBUtil.getConnection(POOLNAME);
		pstmt=conn.prepareStatement("select * from sop_down where seq=?");
		pstmt.setInt(1, seq);
	 ...
	 public List selectDownDtail(int seq_acc) throws MgrException {
        ...
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from sop_down a inner join member b on a.seq_mem=b.mseq where seq_acc=? order by seq desc;" );
