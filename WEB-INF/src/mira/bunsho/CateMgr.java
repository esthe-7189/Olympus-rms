	...
	PreparedStatement pstmtInsertMessage = null;        
	  ...
	  try {
            ...
            if (board.getParentId() ==0) {                 
				...
				rsGroup = stmtGroup.executeQuery("select max(GROUP_ID) from list_cate");                
					int maxGroupId = 0;                
					if (rsGroup.next()) {
				...
				board.setCateNo(maxGroupId);  // (level==1)
                board.setOrderNo(0);
            } else {               
                
				pstmtOrder = conn.prepareStatement( "select max(ORDER_NO) from list_cate "
				...
				board.setCateNo(maxOrder);  //(level==,3)
			}
            
           
            if (board.getOrderNo() > 0) {
                pstmtOrderUpdate = conn.prepareStatement(
					"update list_cate set ORDER_NO = ORDER_NO + 1,CATE_NO = CATE_NO + 1 "	+
            ...
            board.setBseq(Sequencer.nextId(conn,"list_cate"));	
            pstmtInsertMessage = conn.prepareStatement( "insert into list_cate values (?,?,?,?,?,?,?)");
...
public void update(int id, String nm, int  cateno) throws MgrException {
        ...
        try {
            ...
            conn.setAutoCommit(false);
			 ...
			 pstmtMessage = conn.prepareStatement( "update list_cate set NM=?, CATE_NO=?   where BSEQ=?");
	...
	public int count(List whereCond, Map valueMap)   throws MgrException {
        ...
        try {
			...
			query.append("select count(*) from list_cate ");
    ...
    throws MgrException {
        ...
        try {
			...
			query.append("select * from list_cate ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where "); 
 ...
 public Category select(int id) throws MgrException {
        ...
        try {
            ...
            pstmtMessage = conn.prepareStatement(
                "select * from list_cate  where BSEQ = ?");
    ...
    public void delete(int id, int level, int groupId) throws MgrException {
        ...
        try {
			...
			if(level==1){
				pstmtMessage = conn.prepareStatement(
                "delete from list_cate where BSEQ = ? or  GROUP_ID=? ");
				pstmtMessage.setInt(1, id);
				pstmtMessage.setInt(2, groupId);
			}else if(level==2){
				pstmtMessage = conn.prepareStatement(
                "delete from list_cate where BSEQ = ? or PARENT_ID=? ");
				pstmtMessage.setInt(1, id);
				pstmtMessage.setInt(2, id);

			}else if(level==3){
				pstmtMessage = conn.prepareStatement(
                "delete from list_cate where BSEQ = ? ");
				pstmtMessage.setInt(1, id);
			}  
			pstmtMessage.executeUpdate();	
 ...
 public List selectListAdminLevel(int no, int level) throws MgrException {
        ...
        try {            
            ...
            pstmt = conn.prepareStatement(
				"select * from list_cate where PARENT_ID=?  and PLEVEL=?  order by CATE_NO asc" );
            pstmt.setInt(1, no);		
 ...
 public List selectStayL( int level) throws MgrException {
        ...
        try {            
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
				"select * from list_cate where PLEVEL=?  order by GROUP_ID asc " );            
 ...
 public List selectStayMS(int parent_id, int level) throws MgrException {
        ...
        try {            
            conn = DBUtil.getConnection(POOLNAME);            
            pstmt = conn.prepareStatement(
				"select * from list_cate  where GROUP_ID=?  and PLEVEL=?  order by  ORDER_NO asc" );
...
/****************category item count********************/
 ...
 public int  cateCnt(int  id, int level) throws MgrException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            stmt = conn.prepareStatement("select count(*) from list_cate where PARENT_ID=?  and PLEVEL=?");
			stmt.setInt(1, id);	
			stmt.setInt(2, level);	
