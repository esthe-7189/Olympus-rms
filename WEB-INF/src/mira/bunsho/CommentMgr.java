...
public class CommentMgr {
    ...
    private static String POOLNAME = "pool";
	  ...
	  try {
            ...
            if (board.getParentId() ==0) {                 
				...
				rsGroup = stmtGroup.executeQuery("select max(GROUP_ID) from comment");                
					int maxGroupId = 0;                
					if (rsGroup.next()) {
				...
				pstmtOrder = conn.prepareStatement( "select max(ORDER_NO) from comment "
            ...
            if (board.getOrderNo() > 0) {
                pstmtOrderUpdate = conn.prepareStatement(
					"update comment set ORDER_NO = ORDER_NO + 1 "	+
            ...
            board.setBseq(Sequencer.nextId(conn,"comment"));	
            pstmtInsertMessage = conn.prepareStatement( "insert into comment  values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            throw new MgrException("comment Insert", ex);
        } finally {             
  ...
  /*** 수정하기*/ 
        ...
        try {
            ...
            pstmtUpdateMessage = conn.prepareStatement( 
				"update comment set NM=?,MAIL_ADD=?, TITLE=?,CONTENT=?,HENJI_YN=?,OK_YN=?,IP_ADD=?   "+
				"where BSEQ=?");
	...
	public int count(List whereCond, Map valueMap)   throws MgrException {
        ...
        try {
			...
			query.append("select count(*) from comment ");
    ...
    throws MgrException {
        ...
        try {
			...
			query.append("select * from comment ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append("where "); 
	...
	public int count(int fileBseq, int fileKind, List whereCond, Map valueMap)   throws MgrException {
        ...
        try {
			...
			query.append("select count(*) from comment  where FILE_BSEQ=? and FILE_KIND=? ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append(" and "); 
   ...
   public List selectList_item(int fileBseq, int fileKind, List whereCond, Map valueMap, int startRow, int endRow)
    throws MgrException {
        ...
        try {
			...
			query.append("select * from comment  where FILE_BSEQ=? and FILE_KIND=? ");
            if (whereCond != null && whereCond.size() > 0) {
                query.append(" and  "); 
 ...
 public int  HenjiCnt(int bseq, int fileKind) throws MgrException {
        ...
        try {
            conn = DBUtil.getConnection(POOLNAME);
            stmt = conn.prepareStatement("select count(*) from comment  where FILE_BSEQ=? and FILE_KIND=? and  OK_YN=0");
			stmt.setInt(1, bseq);	
...
/*** 조회하기.*/     
        ...
        try {
            ...
            pstmtUpdateMessage = conn.prepareStatement( 
				"update comment set HIT_CNT=HIT_CNT+1 where BSEQ=?");           
...
//comment 패스워드 체크
   public int checkPass(int bseq , String password)   throws MgrException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
                "select PASSNO from comment  where BSEQ = ?");
            pstmt.setInt(1, bseq);
            rs = pstmt.executeQuery();
            if (rs.next()) {
 ...
 public void delete(int id, int level, int groupId) throws MgrException {
        ...
        try {
			...
			if(level==1){
				pstmtMessage = conn.prepareStatement(
                "delete from comment  where BSEQ = ? or  GROUP_ID=? ");
				pstmtMessage.setInt(1, id);
				pstmtMessage.setInt(2, groupId);
			}else if(level==2){
				pstmtMessage = conn.prepareStatement(
                "delete from comment  where BSEQ = ? or PARENT_ID=? ");
				pstmtMessage.setInt(1, id);
				pstmtMessage.setInt(2, id);

			}else if(level==3){
				pstmtMessage = conn.prepareStatement(
                "delete from comment  where BSEQ = ? ");
				pstmtMessage.setInt(1, id);
			}  
